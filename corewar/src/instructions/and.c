...
int *get_and_operator(pc_t *pc, vm_t *vm, champions_t **champ, int i)
{
...
{
    params_t **types = get_type_of_instruction(pc, vm);
    int *and = get_and_operator(pc, vm, champ, i);
    int *to_res = malloc(sizeof(int) * DIR_SIZE);
    int *tmp = malloc(sizeof(int) * DIR_SIZE);

    if (types[1]->type == 3) {
        my_memcpy_rev(to_res, vm->memory + pc->adress
        + 2 + types[0]->size, IND_SIZE);
        my_memcpy(to_res, vm->memory + pc->adress +
        *to_res % IDX_MOD, REG_SIZE);
        pc->adress += IND_SIZE + types[0]->size + 2;
    } if (types[1]->type == 2) {
        my_memcpy_rev(to_res, (vm->memory + pc->adress
        + 2 + types[0]->size), DIR_SIZE);
        pc->adress += DIR_SIZE + types[0]->size + 2;
    } if (types[1]->type == 1) {
        my_memcpy(to_res, champ[i]->reg[vm->memory[pc->adress
